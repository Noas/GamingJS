...
Preloader.prototype.initialize = function()
{
	...
	this.preQueue.loadFile({src:"assets/images/cnec.png", id:"logoCnec"});
};
...
Preloader.prototype.loadContent = function()
{
	...
	container = Container.createContainer("preloader", {width:this.containerWidth, height:this.containerHeight, x:stage.width * 0.5, y:stage.height * 0.5});
	
	// criando o background para o preloader
	var bg = Graphic.createRoundedRect({fillColors:["#EEEEEE","#CCCCCC"], strokeColor:"#CCCCCC", strokeStyle:0.5}, {width:this.containerWidth, height:this.containerHeight, roundness:15});
	bg.x = 200; 
    ...
    graphics.beginStroke("#073B5D").drawRect(25, this.containerHeight * 0.6, this.barWidth, this.barHeight);
    ...
    label = Label.createLabel("Carregando o jogo 0%", textFormat, "#EEEEEE");
...
Preloader.prototype.handleProgress = function(event)
{
    ...
    graphics.beginStroke("#C0C0C0").drawRect(25, this.containerHeight * 0.6, this.barWidth, this.barHeight).setStrokeStyle(0.5);
    // Draw the progress bar
    graphics.beginFill("#008ACA").drawRect(25, this.containerHeight * 0.6, Math.round(this.barWidth * event.progress), this.barHeight);
    // Update the progress
